...
volatile AUDIO_CircularBuffer_t _buffer;
...
BUFFER_RESULT  initBuffer(uint16_t pkgSize, uint16_t numberOfPackages) {
	_buffer.pRead = 0;
	_buffer.pWrite = 0;
	_buffer.slots = CIRC_BUFFER_SIZE;
	_buffer.pData = _actual_buffer;
...
BUFFER_RESULT putBuffer(uint16_t *pcmSamples, uint16_t len) {
	if (!_buffer.Isinitialized)
		return BUFFER_ERROR;

	for (uint16_t x = 0; x < len; x++) {
		_guard_write_and_increment();
		_buffer.pData[_buffer.pWrite] = (pcmSamples[x] & 0xff);
		_guard_write_and_increment();
		_buffer.pData[_buffer.pWrite] = (pcmSamples[x] >> 8);
...
BUFFER_RESULT getBuffer(uint8_t *ptr, uint16_t len) {
	if (!_buffer.Isinitialized)
		return BUFFER_ERROR;
